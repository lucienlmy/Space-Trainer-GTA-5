...
void PopulateVehicleModelsArray()
{
	...
	for (int d = 0x0; d < 0x20; d++)
	{
		for (auto& dd : hashes[d])
		{
			...
			{
				Model m(dd);
				if (!m.IsValid() || !m.IsInCdImage() || !m.IsVehicle())
					continue;
				if (g_isEnhanced || !isMinGameVersion3095 || !IS_VEHICLE_GEN9_EXCLUSIVE_MODEL(dd))
				{
					// Skip known crashy / stub models on Legacy.
					const std::string n = boost::to_lower_copy(m.VehicleModelName());
					static const std::unordered_set<std::string> skipNames = {
						"astron","astron2","cyclone2","ignus2","entity3","virtue","panthere","corsita",
						"baller7","baller8","issi8","aleutian","dorado","impaler5","impaler6","cavalcade3",
						"polterminus","terminus","vivanite","pipistrello","coquette5","envisage","eurosx32",
						"niobe","paragon3","suzume","banshee3","buffalo5","coureur","dominator9","fr36","hardy"
					};
					if (!g_isEnhanced && skipNames.count(n))
						continue;
