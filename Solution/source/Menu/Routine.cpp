...
void Menu::justopened()
{
	...
	if (!GTAmemory::GetIsEnhanced()) {
		...
		{
			//Game::Print::PrintBottomCentre("~r~Warning~s~: 9th Gen content detected, Game may crash. Read SpaceLog for fix instructions.");
			...
			ige::myLog << ige::LogType::LOG_WARNING << "Gen9 Content found in dlcpacks, this can cause instability when attempted to be loaded by Space." << std::endl
		...
		{
			//Game::Print::PrintBottomCentre("~r~Warning~s~: DLCPack mp2024_02 present, Game may crash. Read SpaceLog for fix instructions.");
			ige::myLog << ige::LogType::LOG_WARNING << "mp2024_02 found in dlcpacks, certain bugged clothing can cause instability when attempted to be loaded by Space." << std::endl
...
inline void SpaceMain()
{	
	bool firstTick = true;
	addlog(ige::LogType::LOG_TRACE, "Loading Textures");
	DxHookIMG::LoadAllSpaceTexturesInit();
	...
	for (;;)
	{
		...
		Menu::Tick();
		if (firstTick)
			addlog(ige::LogType::LOG_TRACE, "First Tick - Load Space Config");
		TickSpaceConfig();
		if (firstTick)
...
void ThreadSpaceMain()
{
	...
	addlog(ige::LogType::LOG_TRACE, "Launching SpaceMain");
	SpaceMain();
}
...
void TickSpaceConfig()
{
	...
	if (firstTick)
		addlog(ige::LogType::LOG_TRACE, "First Tick - Run TickSpaceConfig");
	//if (GetTickCount() > g_SpaceConfigOnceTick + 9000U)
	if (GetTickCount() > g_SpaceConfigTick + 30000U)
	{
		if (MenuConfig::bSaveAtIntervals)
		{
			MenuConfig::SaveConfig();
		}
		g_SpaceConfigTick = GetTickCount();
	}
	firstTick = false;
}
...
std::string g_spawnVehiclePlateText = "SPACE";
...
bool g_spawnVehicleAutoUpgrade = false;
bool g_spawnVehicleInvincible = true;
bool g_spawnVehiclePersistent = true;
...
void SetExplosionAtBulletHit(Ped ped, Hash type, bool invisible)
{
	...
	else
	{
		...
		{
			const BOOL net = NETWORK_IS_IN_SESSION() ? TRUE : FALSE;
			ent = CREATE_VEHICLE(type, Pos.x, Pos.y, Pos.z + 0.16f, GET_ENTITY_HEADING(ped), net, net, FALSE);
		}
		else if (IS_MODEL_A_PED(type))
		{
			const BOOL net = NETWORK_IS_IN_SESSION() ? TRUE : FALSE;
			ent = CREATE_PED(PedType::Human, type, Pos.x, Pos.y, Pos.z + 0.16f, GET_ENTITY_HEADING(ped), net, net);
		}
		else
		{
			return;
...
void SetPedInvincibleOn(Ped ped)
{
	...
	// Collision-proof every frame breaks physics and can crash; keep it off.
	SET_ENTITY_PROOFS(ped, TRUE, TRUE, TRUE, FALSE, TRUE, TRUE, TRUE, TRUE);
}
void SetPedInvincibleOff(Ped ped)
{
	...
	SET_ENTITY_CAN_BE_DAMAGED(ped, TRUE);
	SET_PED_DIES_IN_WATER(ped, TRUE);
...
{
	auto ped = PLAYER_PED_ID();
	if (!DOES_ENTITY_EXIST(ped) || IS_PED_IN_ANY_VEHICLE(ped, false))
		return;
	...
	if (!isInAir)
	{
		...
		{
			// Soft stop without freeze toggle (freeze can soft-lock / crash).
...
void DrawGameInfo()
{
	...
	// automatically move the HUD to the left if Space menus are on the right to make sure that neither is obstructed
	...
	case 1: //push out
		for (GTAentity ent : worldEntities)
		{
			...
			if (ent.Handle() == myPed.Handle() || ent.Handle() == g_myVeh)
				continue;
			if (!ent.IsPed() && !ent.IsVehicle())
				continue;
			const Vector3& entPos = ent.GetPosition();
			if (myPos.DistanceTo(entPos) < 10.0f)
			{
				ent.RequestControlOnce();
				ent.ApplyForce(entPos - myPos, ForceType::MaxForceRot2);
			}
		}
		break;
	case 2://explode
		// Prefer natives over fragile memory-bit explosion proof (+392).
		myPed.SetProofs(true, true, true, false, true, true, true, true);
		{
			static int s_lastForcefieldBoom = 0;
			const int now = GET_GAME_TIMER();
			if (now - s_lastForcefieldBoom > 750)
			{
				World::AddExplosion(myPos, EXPLOSION::BLIMP, 5.0f, 0.0f, false, false);
				s_lastForcefieldBoom = now;
			}
		}
		break;
	}
...
void SetWalkUnderwater(Entity PlayerPed)
{
	if (IS_ENTITY_IN_WATER(PlayerPed))
	{
		...
		{
			static int s_lastClearSwimTasks = 0;
			const int now = GET_GAME_TIMER();
			if (now - s_lastClearSwimTasks > 350)
			{
				CLEAR_PED_TASKS_IMMEDIATELY(PlayerPed);
				s_lastClearSwimTasks = now;
			}
		}
	}
}
...
static void TickPlayerAbilities()
{
	...
	if (!DOES_ENTITY_EXIST(myPed) || !IS_PLAYER_PLAYING(myPlayer))
		return;
	...
	{
		SET_PLAYER_INVINCIBLE(myPlayer, TRUE);
		// Re-apply proofs throttled: every-frame SET_ENTITY_PROOFS can crash / desync.
		static Ped s_lastGodPed = 0;
		static int s_lastGodProofsMs = 0;
		const int now = GET_GAME_TIMER();
		if (myPed != s_lastGodPed || now - s_lastGodProofsMs > 450)
		{
			SetPedInvincibleOn(myPed);
			s_lastGodPed = myPed;
			s_lastGodProofsMs = now;
		}
		const int maxHealth = GET_ENTITY_MAX_HEALTH(myPed);
		if (GET_ENTITY_HEALTH(myPed) < maxHealth)
			SET_ENTITY_HEALTH(myPed, maxHealth, 0);
	}
...
void Menu::loops()
{
	...
	// Apply default outfit on first load (only if file exists)
	if (!GET_IS_LOADING_SCREEN_ACTIVE() && !defaultPedSet)
	{
		const std::string defPed = GetPathffA(Pathff::Main, true) + "defaultPed.xml";
		if (does_file_exist(defPed) || does_file_exist("SpaceStuff/defaultPed.xml"))
		{
			sub::ComponentChangerOutfit::Apply(PLAYER_PED_ID(), "SpaceStuff/defaultPed.xml", true, false, false, false, false, false);
			sub::ComponentChangerOutfit::Apply(PLAYER_PED_ID(), "SpaceStuff/defaultPed.xml", false, true, true, true, true, true);
		}
		defaultPedSet = true;
	}
	...
	// Cache current vehicle/weapon before subsystems (Smash/forcefield rely on g_myVeh).
	if (IS_PED_IN_ANY_VEHICLE(PLAYER_PED_ID(), 0))
	{
