...
{
	SpaceExtras::EnsureSpaceStuffFolders();
	...
	const std::string spaceCfg = GetPathffA(Pathff::Main, true) + "spaceConfig.ini";
	if (MenuConfig::iniFile.LoadFile(spaceCfg.c_str()) == 0)
	{
		ConfigRead();
		addlog(ige::LogType::LOG_INFO, "Logging level " + std::to_string(g_loglevel) + " active. Edit loglevel in spaceConfig.ini to change.");
		addlog(ige::LogType::LOG_INFO, "Config loaded from " + spaceCfg);
	}
	else
	{
		addlog(ige::LogType::LOG_ERROR, "Failed to load spaceConfig from " + spaceCfg);
	}

	CustomSkins::EnsureFolder();
	CustomThemes::EnsureFolder();
}
...
void MenuConfig::ConfigRead()
{
	...
	if (menubinds == 0x77 /*F8*/ || menubinds == 0 || menubinds == 0x2D /*Insert*/)
		menubinds = VirtualKey::F5; // F5
	...
	// Keep Space default colours.
	Menu::gradients = false;
	Menu::bit_glare_test = false;
	Menu::bit_centre_title = false;
	Menu::bit_centre_options = false;
	Menu::thinLineOverScrect = true;
	titlebox = { 30, 120, 245, 255 };
	BG = { 0, 0, 0, 200 };
	titletext = { 255, 255, 255, 255 };
	optiontext = { 255, 255, 255, 255 };
	selectedtext = { 255, 255, 255, 255 };
	optionbreaks = { 255, 255, 255, 200 };
	optioncount = { 255, 255, 255, 220 };
	selectionhi = { 30, 120, 245, 255 };
	_globalPedTrackers_Col = { 30, 120, 245, 205 };
	...
	FPSCounter::fpsCorner = (int)ini.GetLongValue(section_haxValues.c_str(), "fps_corner", FPSCounter::fpsCorner);
	if (FPSCounter::fpsCorner < 0 || FPSCounter::fpsCorner > 3) FPSCounter::fpsCorner = 1;
	GraphicsQuality::currentPreset = (int)ini.GetLongValue(section_haxValues.c_str(), "graphics_preset", GraphicsQuality::currentPreset);
	SpaceTheme::uiOpacity = (float)ini.GetDoubleValue(section_haxValues.c_str(), "space_ui_opacity", SpaceTheme::uiOpacity);
	SpaceTheme::uiScale = (float)ini.GetDoubleValue(section_haxValues.c_str(), "space_ui_scale", SpaceTheme::uiScale);
	SpaceTheme::uiBlur = (float)ini.GetDoubleValue(section_haxValues.c_str(), "space_ui_blur", SpaceTheme::uiBlur);
	SpaceTheme::textScale = (float)ini.GetDoubleValue(section_haxValues.c_str(), "space_ui_text_scale", SpaceTheme::textScale);
	SpaceTheme::accentPreset = (SpaceTheme::AccentPreset)ini.GetLongValue(section_haxValues.c_str(), "space_ui_accent", (long)SpaceTheme::accentPreset);
	if ((int)SpaceTheme::accentPreset < 0 || (int)SpaceTheme::accentPreset > 5)
		SpaceTheme::accentPreset = SpaceTheme::AccentPreset::Cyan;
	SpaceTheme::animQuality = (SpaceTheme::AnimQuality)ini.GetLongValue(section_haxValues.c_str(), "space_ui_anim", (long)SpaceTheme::animQuality);
	SpaceTheme::livePulse = ini.GetBoolValue(section_haxValues.c_str(), "space_ui_pulse", SpaceTheme::livePulse);
	SpaceTheme::showTips = ini.GetBoolValue(section_haxValues.c_str(), "space_ui_tips", SpaceTheme::showTips);
	SpaceTheme::showRowDividers = ini.GetBoolValue(section_haxValues.c_str(), "space_ui_dividers", SpaceTheme::showRowDividers);
	SpaceTheme::ApplyDefaultsToLegacyColors();

	// SPACE UI is left-anchored; reset broken right-side offsets from old configs / position slider.
	if (SpaceTheme::spaceUiEnabled)
	{
		if (menuPos.x > 0.10f || menuPos.x < -0.05f)
			menuPos.x = 0.0f;
		if (menuPos.y > 0.28f || menuPos.y < -0.06f)
			menuPos.y = 0.0f;
		if (SpaceTheme::uiScale < 0.75f) SpaceTheme::uiScale = 0.75f;
		if (SpaceTheme::uiScale > 1.25f) SpaceTheme::uiScale = 1.25f;
		if (SpaceTheme::textScale < 0.70f) SpaceTheme::textScale = 0.70f;
		if (SpaceTheme::textScale > 1.30f) SpaceTheme::textScale = 1.30f;
	}
	...
	g_LSCCustoms = ini.GetBoolValue(section_haxValues.c_str(), "vehicle_space_customs_lsc", g_LSCCustoms);
...
void MenuConfig::SaveConfig()
{
	...
	ini.SetLongValue(section_haxValues.c_str(), "fps_corner", FPSCounter::fpsCorner);
	ini.SetLongValue(section_haxValues.c_str(), "graphics_preset", GraphicsQuality::currentPreset);
	ini.SetDoubleValue(section_haxValues.c_str(), "space_ui_opacity", SpaceTheme::uiOpacity);
	ini.SetDoubleValue(section_haxValues.c_str(), "space_ui_scale", SpaceTheme::uiScale);
	ini.SetDoubleValue(section_haxValues.c_str(), "space_ui_blur", SpaceTheme::uiBlur);
	ini.SetDoubleValue(section_haxValues.c_str(), "space_ui_text_scale", SpaceTheme::textScale);
	ini.SetLongValue(section_haxValues.c_str(), "space_ui_accent", (long)SpaceTheme::accentPreset);
	ini.SetLongValue(section_haxValues.c_str(), "space_ui_anim", (long)SpaceTheme::animQuality);
	ini.SetBoolValue(section_haxValues.c_str(), "space_ui_pulse", SpaceTheme::livePulse);
	ini.SetBoolValue(section_haxValues.c_str(), "space_ui_tips", SpaceTheme::showTips);
	ini.SetBoolValue(section_haxValues.c_str(), "space_ui_dividers", SpaceTheme::showRowDividers);
	...
	ini.SetBoolValue(section_haxValues.c_str(), "vehicle_space_customs_lsc", g_LSCCustoms);
	...
	ini.SaveFile((GetPathffA(Pathff::Main, true) + "spaceConfig.ini").c_str());
}
