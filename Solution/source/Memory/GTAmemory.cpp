...
void GTAmemory::InitEnhancedPools() {
	if (g_isEnhanced) {
		...
		// using bit operations. The problem is that Space is loaded a bit too early for the initialization to have happened, and so we can't retrieve those values
	...
	if (!this->bInitialized)
	{
		if (g_isEnhanced) {
			...
			if (this->traxpt == NULL)
			{
				...
				// For that reason, if we don't find it, we look for the new one. (so that we don't break Space for older versions)
...
// Offsets are quite fragile and I don't have a clue when they changed, which might break Space for people using older versions.
...
float GeneralGlobalHax::GetPlayerHeight()
{
	...
	if (!baddr)
		return 1.0f;
	float* ptr = GetMultilayerPointer<float*>(baddr, std::vector<DWORD>{0x8, 0x88});
	return ptr ? *ptr : 1.0f;
}
