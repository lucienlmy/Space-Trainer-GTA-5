...
namespace sub
{
	...
	int SpawnVehicle(GTAmodel::Model model, GTAped ped, bool deleteOld, bool warpIntoVehicle)
	{
		...
		if (MenuTips::IsVehicleSpawnUnsafe(model))
			return 0;
		...
		model.LoadCollision(2500);

		if (oldcarBool)
		{
			float spacing1 = 7.0f;
			Vector3_t d1{}, d2{}, od1{}, od2{};
			GET_MODEL_DIMENSIONS(model.hash, &d1, &d2);
			GET_MODEL_DIMENSIONS(GET_ENTITY_MODEL(oldcar), &od1, &od2);
			const float halfNew = abs(d2.y - d1.y) * 0.5f;
			const float halfOld = abs(od2.y - od1.y) * 0.5f;
			if (halfNew > 0.1f && halfOld > 0.1f)
				spacing1 = halfOld + halfNew + 1.5f;
			if (deleteOld)
		...
		{
			// Spawn slightly in front so the ped isn't fused into the chassis.
			Pos1 = ped.GetOffsetInWorldCoords(0.0f, 6.0f, 0.5f);
		}
		...
		if (oldcarBool && DOES_ENTITY_EXIST(oldcar))
		{
			...
			if (warpIntoVehicle)
			{
				const int maxi = GET_VEHICLE_MAX_NUMBER_OF_PASSENGERS(newcar);
				for (INT i = -1; i <= maxi; i++)
				{
					Ped tempPed = GET_PED_IN_VEHICLE_SEAT(oldcar, i, 0);
					if (DOES_ENTITY_EXIST(tempPed) && GTAentity(tempPed).RequestControl())
			...
			{
				WAIT(0);
				const int maxi = GET_VEHICLE_MAX_NUMBER_OF_PASSENGERS(oldcar);
				for (INT i = -1; i <= maxi; i++)
				{
					Ped tempPed = GET_PED_IN_VEHICLE_SEAT(oldcar, i, 0);
					if (DOES_ENTITY_EXIST(tempPed) && GTAentity(tempPed).RequestControl())
				...
				if (DOES_ENTITY_EXIST(oldcar))
				{
					...
					SET_ENTITY_AS_MISSION_ENTITY(oldcar, true, true);
					DELETE_VEHICLE(&oldcar);
				}
			}
		}
		else if (warpIntoVehicle && DOES_ENTITY_EXIST(newcar))
		{
			SET_PED_INTO_VEHICLE(ped.Handle(), newcar, (int)SEAT_DRIVER);
		}
	...
	namespace VehicleSpawner
	{
		...
		void AddVehicleSpawnOption(const std::string& text, const GTAmodel::Model& vehModel, Ped ped)
		{
			...
			if (MenuTips::IsVehicleSpawnUnsafe(vehModel))
				return;

			bool pressed = false;
			AddOption(text, pressed, nullFunc, -1, false, false);
			MenuTips::SetHoveredTipRaw(MenuTips::GetVehicleSpawnTip(vehModel));
			if (pressed)
			{
				Vehicle vehicle = SpawnVehicle(vehModel, ped, g_spawnVehicleDeleteOld, g_spawnVehicleAutoSit);
				if (!DOES_ENTITY_EXIST(vehicle))
				{
					Game::Print::PrintBottomCentre("~r~Error:~s~ Failed to spawn vehicle.");
					return;
				}
				...
				// Never mark as disposable while seated / auto-sit — causes freeze/crash on sports cars.
				const bool seated = DOES_ENTITY_EXIST(vehicle) && IS_PED_IN_VEHICLE(PLAYER_PED_ID(), vehicle, false);
				if (!NETWORK_IS_IN_SESSION() && !g_spawnVehiclePersistent && !g_spawnVehicleAutoSit && !seated)
				{
	...
	void SpawnVehicleMenu()
	{
		...
		if (spawnRandom)
		{
			...
			if (model.IsInCdImage() && model.IsVehicle() && !MenuTips::IsVehicleSpawnUnsafe(model))
			{
				...
				const bool seated = DOES_ENTITY_EXIST(spawnedVehicle) && IS_PED_IN_VEHICLE(PLAYER_PED_ID(), spawnedVehicle, false);
				if (!NETWORK_IS_IN_SESSION() && !g_spawnVehiclePersistent && !g_spawnVehicleAutoSit && !seated)
	...
	namespace VehicleSearch
	{
		...
		void RebuildResults(const std::string& searchStr)
		{
			...
			{
				if (!model.IsInCdImage() || MenuTips::IsVehicleSpawnUnsafe(model)) continue;
	...
	void SpawnVehicleAllCategoriesMenu()
	{
		...
		{
			if (!vehModel.IsInCdImage() || MenuTips::IsVehicleSpawnUnsafe(vehModel))
			{
				continue;
	...
	namespace VehicleSaver
	{
		...
		void VehicleSaveToFile(std::string filePath, GTAvehicle ev)
		{
			...
			nodeVehicle.append_attribute("space_ver") = SPACE_CURRENT_VER_;
